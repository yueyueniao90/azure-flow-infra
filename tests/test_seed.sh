...
assert_eq "exit code" 0 "$RC"
assert_file_contains "prod group in prod subscription" "$FAKE_AZ_STATE/calls.log" "group create --name rg-azflow-prod --location swedencentral --subscription sub-prod"
assert_file_contains "staging group in staging subscription" "$FAKE_AZ_STATE/calls.log" "group create --name rg-azflow-staging --location swedencentral --subscription sub-test"
